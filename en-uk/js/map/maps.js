...
function outputPopup(dataPartner, i)
	{
		...
		if(i)
			{
				...
				//console.log(JSON.stringify(dataPartner));
			}
		//...end grid view only
		...
		//'<span class="close-infowindow"><img src="/en-uk/img/cross.png" alt="cross"></span>'+
		'<img class="infowindow-badge" src="'+opt.badgeDirectoryBig+'/'+dataPartner.level+'.png" alt="" />'+
		'<h1>'+dataPartner.company+'</h1>'+
		'<p><strong>Location:</strong> '+dataPartner.city+'</p>';
...
function generateInputs(destinationType, filter,filterOptions)
	{
					...
					uniqueDest = arrayUnique(uniqueDest);
					...
					if(uniqueDest[1] && destinationType != 'region'){
						$('.region').parent().show();
					}else{
						$('.region').parent().hide();
					}
						...
						if($(this).is(':checked'))
							{
						 		...
						 		if(filterOptions[0] == 'country'){country.push(filterOptions[1]);}
							...
							for(var i = 0; i< filter.length; i++)	
								{
									...
									'<p class="grid_view_more_link" data-infowindow="'+filter[i].id+'">Find out more &gt;</p></li>';
								}
			    ...
			    $(opt.nav).on('click', opt.resetMap, function()
			    	{
				    	if($("input:checkbox").is(':checked'))
				    		{	// the if statement is mandatory otherwise the pins disappear on a second click of the reset button
			    				...
			    				var t = {'country': ['UK'],'region': ['blank'],'city': ['blank'],'level': ['blank'],'view': ['map_view']};
